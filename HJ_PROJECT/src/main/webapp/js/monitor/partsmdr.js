...
$(function(){
	...
	var lineurl="/soa/echarts/datas/parts?cmd=line";
	var barurl="/soa/echarts/datas/fault?cmd=bar";
	var pieurl="/soa/echarts/datas/parts?cmd=pie";
	var tableurl="/soa/echarts/datas/parts?cmd=table";
...
//		  });
			...
			post(
				...
				function(obj){
					if (obj.success===0) {
						...
						setchartData($('#pie1'), datas, "模块故障状态", "模块故障状态");
	    ...
	    var btoption = {      
	    		tooltip: {
		            formatter: function(params) {
		                ...
		                var str= '<p style="color:#fff600;text-align:center;font-size:16px;">' + o.name + '</p>';
		                str += '<p style="color:#fff600;text-align:center;font-size:16px;">' + o.f + '</p>';
		                str += text1 + '：<span style="color:#8aff00;">' + o.zx + '</span><br/>';
		                str += text2 + '：<span style="color:#8aff00;">' + o.zs + '</span>';
		                return str;
	  ...
	  //饼图右边列表 数据 模板
	    ...
	    function settextfn(o, data){
	    	...
	    	var html = '';
	    	var title='当前全省模块共计<font color="red">'+data.length+'</font>个, ';
	    	var li='';
	    	$.each(data, function(index, el) {
            	var sumave=el.totalNumber>0?((el.faultNumber/el.totalNumber).toFixed(2)*100):0;
	  ...
	  //-------------auto-----s
	   ...
	   //-------------auto-----e
	    ...
	    //导出
	    $('#partsExport').on('click',function(){
	    	location.href =contextPath+'/soa/echarts/parts/export?oid='+oid;
	    });
