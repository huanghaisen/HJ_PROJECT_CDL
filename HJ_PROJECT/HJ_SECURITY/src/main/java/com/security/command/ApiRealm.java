...
//        SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
		...
		if(token!=null && !"".equals(token)){
			...
			user=securityService.getUserName(username);
			String MD5PASSWORD ="";
			if (userFrom!=9) {
				MD5PASSWORD = Md5Encoder.getMd5Msg(username+password+StringUtil.getSimpleDateFormat(user.getCreateTime()));//登陆提交上来的用户名和密码
			}else{
