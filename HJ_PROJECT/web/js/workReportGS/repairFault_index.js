...
var table = $('#table_form').DataTable({
		...
		'rowCallback' : function(row, data, dataIndex) {
			//获取行id
			var rowId = data[0];
			var num =parseInt(data.faultsum,10)-parseInt(data.procsum,10);
			var sum=parseInt(data.faultsum,10);
			var rount=sum==0?"0%":(num/sum).toFixed(2)*100+"%";
			$(row).find('td').eq('3').html(num);
			$(row).find('td').eq('5').html(rount);
			// 行已经在选择行的组里面
